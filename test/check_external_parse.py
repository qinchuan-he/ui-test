...
def check_pdf_parse():
    try:
        ...
        stdin, stdout, stderr = ssh_client.exec_command('docker cp  {}:{} {}'.format(container_id,inner_container_file_path,ServerInfo().server_folder_path))
        msg = stdout.read().decode('utf-8')
        # 获取文件中内容
        current_day = time.strftime('%Y-%m-%d',time.localtime(time.time()))
        stdin, stdout, stderr = ssh_client.exec_command("cat {} | grep {} | grep 'request_user_info' | awk '{{print {}}}'"
...
def send_msg():
    ...
    content_start = '<html><header><h1>PDF解析调用0次,ocr服务调用0次</h1><h3>调用明细如下：</h3></header><body><table border="1">'
    ...
    msg = check_pdf_parse()
    if msg:
        content_start = '<html><header><h1>PDF解析调用{}次,ocr服务调用0次</h1><h3>调用明细如下：</h3></header><body><table border="1">'.format(len(msg)-1)
        for i in range(len(msg)-1):
            try:
                tr = msg[i].split(" ")
                content_body=content_body+'<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(i+1,tr[0],tr[1],tr[2],tr[3])
            except Exception as e:
                print(i)
                print(e)
    msg_2 = check_ocr()
    if msg_2:
        content_start = '<html><header><h1>PDF解析调用{}次,ocr服务调用{}次</h1><h3>调用明细如下：</h3></header><body><table border="1">'.format(len(msg)-1,len(msg_2)-1)
